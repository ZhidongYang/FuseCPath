...
# zhidong yang 20250603 test mlp_dim=768, aligned with the dim in gigapath teacher embeddings
class RRTMIL_MVKD(nn.Module):
    def __init__(self, input_dim=1024,mlp_dim=512,act='relu',n_classes=2,dropout=0.25,pos_pos=0,pos='none',peg_k=7,attn='rmsa',pool='attn',region_num=8,n_layers=2,n_heads=8,drop_path=0.,da_act='relu',trans_dropout=0.1,ffn=False,ffn_act='gelu',mlp_ratio=4.,da_gated=False,da_bias=False,da_dropout=False,trans_dim=64,epeg=True,min_region_num=0,qkv_bias=True,**kwargs):
        super(RRTMIL_MVKD, self).__init__()
        ...
        self.teacher_projector = nn.Linear(768,self.online_encoder.final_dim)

        self.predictor = nn.Linear(self.online_encoder.final_dim,n_classes)

        self.apply(initialize_weights)

    def forward(self, x, role='student', return_attn=False,no_norm=False):
        if role == 'student':
            x = self.patch_to_emb(x) # n*512
            x = self.dp(x)
...
# zhidong yang 20250603 test mlp_dim=768, aligned with the dim in gigapath teacher embeddings
class RRTMIL_COLMVKD(nn.Module):
    def __init__(self, input_dim=1024, mlp_dim=512, act='relu', n_classes=2, dropout=0.25, pos_pos=0, pos='none',
        ...
        super(RRTMIL_COLMVKD, self).__init__()
        ...
        self.teacher_projector_1 = nn.Linear(768, self.online_encoder.final_dim)

        self.teacher_projector_2 = nn.Linear(768, self.online_encoder.final_dim)

        self.teacher_projector_3 = nn.Linear(1280, self.online_encoder.final_dim)
    ...
    def forward(self, x, role='student', return_attn=False, no_norm=False):
        if role == 'student':
            ...
            x = self.teacher_projector_1(x)
            # prediction
            logits = self.predictor(x)
        elif role == 'teacher_2':
            x = self.teacher_projector_2(x)
            # prediction
            logits = self.predictor(x)
        elif role == 'teacher_3':
            x = self.teacher_projector_3(x)
            # prediction
            logits = self.predictor(x)
