...
def train_one_epoch_mvckd_collaborated(model, mil_model, alpha, T, optimizer, data_loader, device, epoch):
    model.train()
    ...
    for step, data in enumerate(data_loader):
        ...
        if mil_model in ('clam', 'dsmil'):
            train_logits, cls_loss, patch_num = model(images.to(device), labels, role='student')
